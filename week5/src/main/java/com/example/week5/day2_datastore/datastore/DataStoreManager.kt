...
class DataStoreManager @Inject constructor(                  // here, context is required, but hilt doesn't know to how to provide it,
   @ApplicationContext private val context: Context         // you can provide context in hilt by two methods 1. @Provide 2. @ApplicationContext
    ...
    val authenticatedKey = booleanPreferencesKey(name = "authenticated_key")
    val authenticated : Flow<Boolean> = context.dataStore.data.map { preferences ->      // Flow is used bcz whenever the value of authenticatedKey changes, authenticated read and write dynamically,
        preferences[authenticatedKey]   ?: false                        //  bcz this key may or may not present in the datastore. that's why we check key == true.
    }
    ...
    suspend fun logout() {
         context.dataStore.edit { preferences ->
             preferences.remove(authenticatedKey)
         }
    }
}